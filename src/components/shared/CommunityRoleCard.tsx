...
interface CommunityRoleCardProps {
...
const CommunityRoleCard = ({ onRoleToggle, selectedRoles }: CommunityRoleCardProps) => {
  ...
  const toggleRole = (id: string) => {
    setCommunityRoles(prevRoles =>
      prevRoles.map(role => {
        if (role.id ===id) {
          const newSelected = !role.selected;
          onRoleToggle(role.id, newSelected, role.label, 'community'); // Pass to parent
  ...
  const toggleCustomRole = (id: string) => {
    setCustomRoles(prevRoles =>
      prevRoles.map(role => {
        if (role.id === id) {
          const newSelected = !role.selected;
          onRoleToggle(role.id, newSelected, role.label, 'community'); // Consistency
          return { ...role, selected: newSelected };
        }
        return role;
      })
