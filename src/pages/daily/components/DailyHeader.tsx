    ...
    <div className="bg-white rounded-xl shadow-sm p-4">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-secondary-50 p-3 rounded-lg text-center">
          <h4 className="text-sm text-gray-600 mb-1">Authentic Deposits</h4>
          <p className="text-xl font-semibold text-secondary-700">
            {dailyStats.authenticDeposits.completed}/{dailyStats.authenticDeposits.total}
          </p>
          <div className="w-full bg-gray-200 rounded-full h-1.5 mt-2">
            <div 
              className="bg-secondary-600 h-1.5 rounded-full" 
              style={{ width: `${(dailyStats.authenticDeposits.completed / dailyStats.authenticDeposits.total) * 100}%` }}
            ></div>
          </div>
        </div>
